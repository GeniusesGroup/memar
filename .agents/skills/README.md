...
## One practice into one model — not a skill catalog
Memar exposes a **single** agent practice under `skills/memar/`. That practice discovers and applies the live documentation tree; it does not embed project knowledge and it does not compete with peer "Memar skills" for Khayyam, Cognition, or other sub-subjects.

Splitting those subjects into separate skills was tried and rejected: Khayyam has no stand-alone operating practice without the rest of Memar; Cognition belongs as an Explanation (and Practice) document in `docs/`, not as a second product surface. The continuous mental model lives in documentation; the skill is the entry, not the encyclopedia ([Knowledge → A project carries one continuous mental model](../docs/knowledge.md#a-project-carries-one-continuous-mental-model)). Installing that entry into another project is `../scripts/install-agents.py` ([AGENTS.md](https://agents.md/)); installing it into local agent apps is `../scripts/install-apps.py`. `--help` is the interface, not a host-tool plugin catalog.
