...
A capsule is an abstraction layered over `vr` (variable) and `mt` (method), not an independent primitive. Accordingly, the mechanical grammar of the method signature itself is specified in [Method in Khayyam](./khayyam-method.md#method-as-callable-capsule), and the `sc` (code scope) mechanism underlying control-flow libraries is specified in [Control Flow in Khayyam](./khayyam-control_flow.md#code-scope) — this document references both rather than restating them.
