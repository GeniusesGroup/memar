...
* A **relation between instances of the same concept** — an edge may join two nodes of one concept while carrying semantics that hold only instance-to-instance: one `Transaction` guarantees another, a record replaces or is conditional on a predecessor, one version of a document supersedes the next. A recurring pattern deserves naming here: a higher-level concern can be nothing but the structure of such relations among instances of a lower concept — without itself being another instance of that concept, and without being a container owning the instances it relates.
...
It follows that a correction is recorded the same way a change is: a fix or reversal of a previously recorded effect is itself a new fact that references the effect it corrects, never a deletion or rewriting of the history. The realized state is derived from the whole actual history — corrections included — and erasing the superseded fact would discard the very distinction this section exists to preserve: that a correction happened, when, and to what.
...
Targeting resolves at the level where the constraint originates: a constraint may name its target one level above where it will be realized — against a person or an organization, say, rather than against one of that person's many accounts — and the specific lower-level place of realization may be determined later. Forcing a constraint to reference a not-yet-known resource early binds it to the wrong node and reproduces, in advance, the per-resource duplication this section rejects.
