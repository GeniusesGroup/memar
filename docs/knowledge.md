...
**Scope boundary.** This document owns the concept of knowledge and the modeling principles below. It does not own: the process of thinking ([Cognition](./cognition.md)); the structure of acting systems ([Agency](./agency.md)); the domain model of Content and Relation ([Content](./content.md), [Modeling](./modeling.md)); or knowledge management as an organizational discipline — creation workflows, tacit-to-explicit conversion practice, sector adoption patterns, and change management — which belongs to the Organization project.
...
**Not everything that agents need should be developed as human-readable prose.** Some knowledge is more useful as an **executable projection**: a language implementation, a generator, a language-server-style process, or a tool surface an agent can call (for example, asking a Memar language server to produce the concrete error types for a named module). In that division of labor:

- **Documentation** states the contract and the rules for developing and using the module — what the executable surface must mean, guarantee, and expose.
- **Code services** perform the generative or mechanical work — the agent requests outcomes; it does not follow a Practice that narrates every file path and every concrete error by hand.

Writing an ever-longer Practice that tells an agent where to write each error (or equivalent mechanical artifact) is the wrong home for that knowledge: it fights model progress, duplicates what a generator should own, and pretends prose is a substitute for a tool. The Error protocol and its local implementation modules are the pattern: concepts and contracts in `docs/`; generation in callable modules under [`modules/`](../modules/) that the agent invokes.
...
**Principle statement (software instance):** Treat prose documentation and executable modules as complementary projections of the same knowledge. Put generative and mechanical work in callable code services; put lasting contracts and development rules for those services in documentation — not the reverse.

#### A Project Carries One Continuous Mental Model
A project's knowledge is one continuous mental model — the definitions, relationships, and practices that make work inside that project coherent. Delivery channels (a Skill file a host tool knows how to discover, a pointer in a consuming repository's agent-instruction file) are **projections of access**, not partitions of the model.

A recurring ecosystem pattern does the opposite: the project introduces itself differently to each harness; each introduction may advertise several plugins; each plugin may ship several skills; each skill carries a fragment. The result looks installable and feels modular, but the *model* is no longer continuous. An agent that loads one fragment cannot hold the practice the fragment presupposes. Memar's own trials of that shape failed for the same reason: a separate Khayyam skill, and Cognition as a peer skill beside Memar, both collapsed back into internal documentation — Khayyam has no coherent practice without the rest of Memar; Cognition is a document in the shared medium, not a second product surface.

Memar does not package itself as host-tool plugins or marketplace cards. A consuming project installs a pointer into its own instruction files; optionally the same skill folder is copied into user-level skill directories of agent apps on a machine. The skill is a routing practice into the live `docs/` tree, not a second copy of it. One authoritative `docs/` tree (plus the implementation modules in this repository that realize its contracts) is the model.

**Principle statement:** A project's internal knowledge is one continuous mental model. Access projections may multiply; the model must not. Do not split a framework into peer skill packs that cannot stand alone.

#### Write for the Structure of the Subject, Not for Today's Reader Limits
Cognitive systems that read documentation — humans and AI models — change capability quickly. Text written to paper over a *current* reader weakness (persuasion against rationalization, table layouts that look clear only when rendered as a grid, digraphs that only help if a Graphviz viewer is present, rituals that exist because last year's agent skipped steps) ages into noise when the next reader no longer has that weakness — or never parses the layout the way a human eye does.

Documentation that earns a lasting place states what is true of the **subject**: definitions, relationships, constraints, and procedures that remain correct when models are faster, calmer, or differently tokenized. Workarounds for a temporary reader failure mode belong in ephemeral practice notes or harness adapters, not in the governing record. Prefer linear structure (headings, numbered steps, explicit sentences) for load-bearing logic; do not encode norms in decorative two-dimensional layouts that models misread as flat token streams.

This principle is the knowledge-side face of Cognition's preference for long-term clarity ([Cognition → Discourse Norms](./cognition.md#discourse-norms-derived-from-this-model)).

**Principle statement:** Write governing documentation for the structure of the subject as it is understood, not for the temporary limits of today's readers. Representation that only works for one rendering or one model generation is not durable knowledge.
