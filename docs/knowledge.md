...
- **Documentation** states the contract and the *how to develop the module across languages* — what the executable surface must mean, guarantee, and expose.
...
Writing an ever-longer Practice that tells an agent where to write each error (or equivalent mechanical artifact) is the wrong home for that knowledge: it fights model progress, duplicates what a generator should own, and pretends prose is a substitute for a tool. The Error protocol and its language realizations are the pattern: concepts and contracts in `docs/`; generation in `memar-{language}` (and kindred) services the agent invokes.
...
Memar does not package itself as host-tool plugins or marketplace cards. A consuming project installs a pointer into its own instruction files; optionally the same skill folder is copied into user-level skill directories of agent apps on a machine. The skill is a routing practice into the live `docs/` tree, not a second copy of it. One authoritative `docs/` tree (plus the language implementations that realize its contracts) is the model.
