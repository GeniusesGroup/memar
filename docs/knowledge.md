...
A project's knowledge is one continuous mental model — the definitions, relationships, and practices that make work inside that project coherent. Delivery channels (an IDE plugin, a CLI marketplace card, a Skill file the host tool knows how to discover) are **projections of access**, not partitions of the model.
...
Marketplace and plugin packaging may still exist as **distribution tax** paid to host tools ([Installing](../.agents/installing.md#what-marketplace-and-plugin-are-not)). They must not become the way the project thinks about its own knowledge. One authoritative `docs/` tree (plus the language implementations that realize its contracts) is the model; adapters discover it.

**Principle statement:** A project's internal knowledge is one continuous mental model. Distribution adapters may multiply; the model must not. Do not split a framework into peer skill packs that cannot stand alone.
