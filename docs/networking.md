...
### Frames
Frames indicate in each protocol's own RFC — e.g. [GP](./networking-osi_3-Giti-Network.md), [sRPC Protocol](./sRPC.md), [Chapar](./chapar.md). Each packet can carry many frames as long as it respects the network MTU. All frames have a fixed first field named `FrameType`, one byte wide. It shares its *concept* with a [MediaTypeID](./media-type.md) — a registered identifier naming what follows — but deliberately not its width, and the reason is worth recording so the question does not resurface: a MediaTypeID is 64 bits because it is hash-derived (the first 64 bits of SHA3-256 over the structure), letting anyone mint types without a registrar, where width buys collision resistance. A FrameType is the opposite bargain: a small, centrally-registered set of wire framings maintained in one table here — and it appears in every frame of every packet, so its width is paid continuously by all traffic. One byte covers the real population with room for an experimental range and a signed-extension escape hatch; the two registries stay separate because their allocation models differ, not merely their widths.
...
|             4              |          GP          | [Protocol](./networking-osi_3-Giti-Network.md) |
