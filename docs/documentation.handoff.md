...
- State: the rule that fundamental concepts must not depend on derived mechanisms — Polymorphism may cite Modeling and Type, never the reverse; base documents may not cite the protocol layer projected onto them; Terminology stands independent of other documents — is applied case by case but has never been stated as a named documentation convention. The polymorphism session proposed it for a conventions file; that file was later retired into this document's meta layer, and the convention itself never followed it home.
- Next: decide whether to register it as a convention in [documentation.md → Citations](./documentation.md#citations) (or a sibling section) or to leave it as applied-only practice re-derived at each review.
...
### Practice/agent-configuration split
- State: recovered 2026-09-17 from an audit of retired chats-context files (`project instruction and skills - chats.md`, `ساختار-پلاگین-برای-Agent.md`). A documentation-architecture session reached a three-way separation — **Documentation** = the project's truth (what), **Practices** = how that truth is worked with (how, valid for any consumer: human, AI, IDE, automation), **Agent Configuration** = how the two are introduced to a particular agent/platform. The session's acceptance test: if a practice would still be worth keeping when no AI agent exists, it is documentation; if it exists only to satisfy one agent's limitations, it belongs in agent configuration, never in `docs/`. The repository has been *operating* on this separation ever since (Practice-facet documents live in `docs/`, tool-dependent discovery lives in `.agents/skills/`, and the retired `.agents/skills/main/` skill's rules were absorbed into [Cognition](./cognition.md)), and the same separation is stated operationally in the memar skill's own "Separation of knowledge" — but no Memar document owns the separation as a stated principle with its acceptance test.
- Next: when the documentation documents are next revised, state the split in the Practice facet's governing specification ([documentation-practice.md](./documentation-practice.md)) or [documentation.md](./documentation.md), with the no-agent-existence test as the boundary criterion; the complementary Content-vs-Practice framing (a practice is content taking a role, not an intrinsic artifact type — from the same sessions) belongs with it.
