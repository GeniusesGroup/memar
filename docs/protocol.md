...
### Methodology
This document's core ontological claims — the Protocol–Process–System relationship, the Protocol–Science–Methodology connection, and the critique of "standard" as third-party attestation — were drafted by Omid Hekayati, then reviewed and enriched across several rounds by ChatGPT and Super Z (GLM), including a full split of an earlier monolithic document into this document, the Explicit Behavior Ownership document, and the Khayyam inheritance-rejection document, so that ontological questions about Protocol would not stay entangled with unrelated design and language-level decisions. Claude's later review pass corrected process cardinality, distinguished maturity attestation from institutionalization attestation (grounded in IETF RFC 6410), and flagged System and Process as then-undefined foundational dependencies — both of which are now resolved: System and Process each have their own document. Full provenance for every round is recorded in [protocol.changelog.md](./protocol.changelog.md) rather than here.
...
- **Terminology Governance document:** A dedicated document examining how Memar defines and governs its terminology, including the mechanism by which document definitions take precedence over colloquial or ecosystem usage when ambiguity arises. This interacts with the Framework document's document-authority principles.
