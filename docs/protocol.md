...
Memar needs a stable, precise definition of Protocol because the concept serves as a foundational building block. If the term remains ambiguous, every subsequent document that references protocols inherits that ambiguity. By establishing a clear ontological foundation now — one grounded in the relationship between protocols, processes, and systems — we prevent conceptual leakage between related but distinct concerns such as behavior ownership, inheritance, and conformance, each of which has its own document.

### Methodology
This document's core ontological claims — the Protocol–Process–System relationship, the Protocol–Science–Methodology connection, and the critique of "standard" as third-party attestation — were drafted by Omid Hekayati, then reviewed and enriched across several rounds by ChatGPT and Super Z (GLM), including a full split of an earlier monolithic document into this document, the [Explicit Behavior Ownership principle](./type.md#explicit-behavior-ownership), and the Khayyam inheritance-rejection document, so that ontological questions about Protocol would not stay entangled with unrelated design and language-level decisions. Claude's later review pass corrected process cardinality, distinguished maturity attestation from institutionalization attestation (grounded in the IETF's [RFC 6410](https://datatracker.ietf.org/doc/html/rfc6410)), and flagged System and Process as then-undefined foundational dependencies — both of which are now resolved: System and Process each have their own document. Full provenance for every round is recorded in [protocol.changelog.md](./protocol.changelog.md) rather than here.
...
- **Process**: The activity or sequence of activities the rules govern. A protocol without a process is rules on a shelf — a collection of statements with no domain of application. See [Process](./process.md) for the full treatment of what a process is, including why it is not limited to a single, flat sequence; a Protocol may govern one process, several independent processes, or a process at any level of nesting.
...
The relationship between protocol, process, and system is not merely categorical — it is foundational. The general theory of what a System and a Process are, and how they relate to each other, is covered in full in [System](./system.md) and [Process](./process.md); this section covers only what is specific to Protocol's place in that relationship.

Protocol is the third term in the chain:
...
A Protocol, in the sense defined by this document, is the underlying, named set of declarative rules that govern the process those endpoints participate in — the ordering constraints, the required message content, the state transitions, the error semantics — independent of which document format is used to write those rules down, and independent of which programming language or framework will eventually implement them.
...
3. **A protocol can exist, and be reasoned about, before any API specification for it has been written.** Because a Protocol is methodologically produced (see "Protocol, Science, and Methodology," above) and is process-bound rather than document-bound, Memar contributors can define and critique a protocol's rules directly — as this document does for the general concept of Protocol itself — without first committing to an API specification format. The specification should be derived from an already-validated protocol, not the reverse; a protocol should not be inferred backward from whatever specification format a team happened to reach for first. This is the Concept-First-Thinking discipline the Terminology document establishes generally, applied to the specific case of API design.
...
- **Process-bound:** A Protocol governs one or more processes within a system, at any level of process nesting — see [Process → Process Composition](./process.md#process-composition) for what nesting means. A protocol with no process it governs, at any level, is not a protocol — it is inert text. This is the property that distinguishes protocols from general rule sets like style guides or personal preferences.
