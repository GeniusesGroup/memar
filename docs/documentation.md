...
# Documentation
This document defines what a Facet is, names the five facets currently in use, explains how the facet system extends, and carries the cross-cutting conventions that span facets (file naming, documentation language, written surface, results routing, citations, URI). It does not itself specify the structure of any facet — that is each facet's own document's job.

## Abstract
Documentation in this project is organized into **Facets** — distinct kinds of documentation content, each with its own governing structure and its own specification document. A facet is defined by the reader's relationship to the content: what the reader is expected to do with it. This project currently defines five facets — **Explanation** (content meant to be studied to understand something), **Practice** (content meant to be followed to accomplish something), **Changelog** (content meant to be consulted to audit how an artifact changed over time), **Handoff** (content meant to be read to resume a paused discussion across a session boundary), and **Research** (content meant to be examined to evaluate a deliberate inquiry). See [Facets currently defined](#facets-currently-defined) for the definitions and [Extensibility](#extensibility) for how new facets may be added. Cross-cutting conventions that apply regardless of facet — how a document's name encodes its category, concept, and facet; the language the written record is kept in; written surface (no blank line between a section title and its body; linear form for load-bearing logic); where a result of each type is recorded; citation practice; URI form — are defined here as well. The followable writing procedures live in [documentation.practice.md](./documentation.practice.md). These surface rules are about *writing*, not about any one markup protocol.
## Introduction
...
Documentation content must be current truth, not archaeology. Four rules apply to every Explanation- and Practice-facet document (and to pointer files such as directory READMEs that guide readers into those documents):
...
### Written surface
Cross-cutting mechanics of how written documentation is laid out — independent of facet and **independent of any particular markup protocol**. Today's files may use Markdown; tomorrow's may not. These rules govern the *writing*, not a syntax. The followable steps live in [documentation.practice.md](./documentation.practice.md).

- **No blank line between a section title and its body.** The body of a section begins on the line immediately after the section title. A blank line after a title falsely signals a paragraph break and detaches the body from the section it belongs to. One blank line *between* successive blocks inside a section (paragraphs, lists, quotations) remains ordinary paragraph separation; that is not the same as a blank line after the title itself.
- **No run of two or more blank lines** anywhere in the document. Double blanks are decorative noise.
- **Load-bearing logic is linear.** Normative rules, decision procedures, and conditionals are written as sentences or numbered steps under section titles. Do not encode norms in grid layouts or diagram blocks that only make sense when rendered as two-dimensional graphics — those forms are hostile to linear readers (including models) when they carry rules. Short non-normative lookup grids remain allowed where no rule lives in cell adjacency. The knowledge-side rationale is [Knowledge → Write for the Structure of the Subject](./knowledge.md#write-for-the-structure-of-the-subject-not-for-todays-reader-limits).
- **Present-state writing when no prior snapshot is available to the reader.** Until a durable prior snapshot of the document exists that the reader can open (a committed revision, a published release, a frozen research), the body states current findings and decisions as present content. Do not narrate "revised," "updated after review," or "formerly said X" when that former text is not available to the reader — that forces them to ask what changed when they cannot see it. Diff and changelog exist for that job once a prior snapshot exists.
- **The body affirms what holds.** A body sentence states the current rule, definition, or claim in the form a reader applies. The wording that was replaced, the label that was dropped, and the alternative that was rejected are changelog content: the outcome in `What changed`, the rejected alternative in `Considered and not done`. A sentence whose job is to warn the reader off a previous formulation — "X is retired," "Y is not the rule," "this document once said Z" — is that changelog content sitting in the body. A reader who loads the body without the changelog, including an AI-assisted reader, treats the negation as a live claim; [Relevance discipline](./documentation-explanation.md#relevance-discipline) names the same hazard for considered-and-not-done. A prior snapshot does not move that sentence into the body: the snapshot changes whether the old text can be opened, and the changelog is still where the removal is recorded.
