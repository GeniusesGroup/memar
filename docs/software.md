...
| --- | --- | --- |
| Software requirements specification | IEEE Std 830 | [Modeling](./modeling.md) owns which concepts exist; a requirements body is largely modeled concepts and constraints ([System → Structure](./system.md#structure)). Full treatment owed. |
| Software design description | IEEE Std 1016 | Design lands as Type decisions ([Type](./type.md), [Modularity](./modularity.md), the Khayyam documents). |
| Software configuration management | IEEE Std 828 | The framework's Immutable Infrastructure protocol document owns the deployment immutability rule; the lifecycle working-out is owed (see its handoff). |
| Software test documentation | IEEE Std 829 | No home yet; owed. |
| Software verification and validation | IEEE Std 1012 | Partially anchored: abstraction conformance ([Abstraction in Khayyam](./khayyam-abstraction.md)) is the compile-time verification face; the rest owed. |
| Software quality assurance | IEEE Std 730 | No home yet; owed. |
| Software project management | IEEE Std 1058 | No home yet; owed. |
| Software user documentation | IEEE Std 1063 | Partially owned: [Type Practice → Carrying Type metadata](./type.practice.md#carrying-type-metadata) covers human-facing companion artifacts per Type. |
| Software review and audit | IEEE Std 1028 | The documentation system's audit trail ([Changelog facet](./documentation-changelog.md)) is the artifact-history face; the review process owed. |

### "Software ecosystem" is not a Memar term
The word "ecosystem" — used loosely across industry and even in this project's discussions (technology ecosystems, AI ecosystems) — has no Memar definition and must not carry weight in Memar documents until one is decided; until then it carries its ordinary meaning ([Terminology → The Default Meaning of an Unreferenced Term](./terminology.md#the-default-meaning-of-an-unreferenced-term)). Where the document set needs the notion, it names the concrete relationships instead: the base document set, the framework's protocol documents, and the implementation repositories.
