...
- Cited:
  ...
  - [Encapsulation in Khayyam](./encapsulation.md) — Depends_on: Library-Defined Control Flow's ELSE-must-reference-its-condition rule is a direct application of that document's Closures as Implicit Capsule Syntax topic (no implicit binding to 'whatever came before').
  - [Method in Khayyam](./method.md) — Depends_on: Control flow (IF/ELSE, OnPresent/OnAbsent) is built entirely from ordinary method calls, and inherits that document's pass-by-reference, explicit-output-variable, no-chaining model rather than introducing any dedicated call syntax of its own.
...
- Type: Fixed
- Cited:
  - [Method in Khayyam](./method.md) — Depends_on: `sc` entry/exit and the `IF`/`ELSE` exclusivity are learned from `sc` events
...
### Cross-reference to the general grammar-refusal principle
...
#### What changed
- The *Error Propagation* topic now names the absence of dedicated error syntax as a rule rather than an omission, pointing to [The Grammar Refuses Protocol Semantics](./khayyam.md#the-grammar-refuses-protocol-semantics) in the language overview, where the general criterion is stated once; this document keeps the worked case and restates none of it.
