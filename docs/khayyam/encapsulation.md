...
Even trivial read access to a field (e.g., getting a configuration value) requires a method definition. For capsules with many fields, this can result in a large number of getter-style methods. However, these methods serve a critical purpose: they define the capsule's behavioral contract, and they can be evolved independently of the internal representation without breaking consumers. A [Linter](../protocols/linter.md) or companion generator MAY write `get`/`set` methods into explicit source **when requested**; it MUST NOT synthesize a public surface the author never asked for. Mechanical accessors for every field recreate public fields with extra steps. Direct field read/write outside the capsule's own methods is already impossible at the language level; any remaining diagnostic is governance, not a compiler error.
