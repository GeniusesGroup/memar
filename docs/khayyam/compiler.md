...
- **Event abstraction.** The compiler, as an independent application, emits control-flow events (entering/leaving an `sc`, taking/skipping a branch) to which analysis libraries (DAA, linter) subscribe. DAA therefore learns branch exclusivity from `sc` events, not from recognizing `IF` by name. See [Execution Primitives: The Compiler's Role](./control_flow.md#execution-primitives-the-compilers-role) for the full contract.
