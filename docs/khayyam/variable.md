...
The name must be a valid identifier. The type must be a previously defined or imported type. Of the four Type categories (see [Type in Khayyam](../type.md)), a `vr` declaration names a **capsule** or an **abstraction**: those are the categories with an instance for the name to reference. A method's influencing group is wider than a `vr` declaration. It may receive a `vr`, an `sc`, or an `mt`, and the compiler distinguishes the three ([Method → Type-level arguments for `sc` and `mt`](./method.md#type-level-arguments-for-sc-and-mt)). How an abstraction-typed variable resolves to a concrete capsule instance at runtime (dispatch) is specified in [Polymorphism in Khayyam](./polymorphism.md), not in this document. A variable declaration is a distinct, separate statement — it cannot be combined with any other declaration on the same line, and it cannot carry an initializer inline. This ensures that every variable has its own visible scope in the source and that a reader can never skim past a declaration because it was compressed alongside others.
