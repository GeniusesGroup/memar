...
- `IfFailed`/`IfSucceeded` as global, package-less functions — rejected because Khayyam has no package/namespace concept ([Modularity in Khayyam](../khayyam/modularity.md)), so a global function name carries no inherent domain context; it is unclear from the name alone what is being checked for failure/success without an explicit receiver.
