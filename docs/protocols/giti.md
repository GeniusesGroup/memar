...
GP is simple enough to encode and decode in a language-neutral module. The repository's Khayyam modules are the current local realization surface for this protocol; the protocol itself remains language-independent. External projects may realize it in the language and runtime that fit them. GP can ship inside an OS, device drivers, or applications because its specification is small. The software routers of this ecosystem differ in internal service call, security, pipelining, and multiplexing, and are based on code-generation development while remaining usable through plain package APIs.
