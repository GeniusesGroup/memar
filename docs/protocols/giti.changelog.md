...
#### Deliberation
- The migration of this protocol to the current method was directed, and an explicit ruling on the filename was asked for — prefix removal vs. umbrella prefix vs. keeping the OSI-layer reference (Omid Hekayati).
- `giti.md` was recommended (Super Z).
...
#### Deliberation
- Documentation-production history belongs to this changelog, not the base document's Methodology, which now tells how the protocol design was actually reached (Omid Hekayati — decided).
- The commercial statement was defined: ChaparKhane and Achaemenid are intended commercial software from Geniuses Group, and unexplained mentions exist for that reason; organizations stay free to build their own implementations on the open protocols, but support rather than self-implementation is the path that speeds Memar's development (Omid Hekayati — decided).
...
#### Deliberation
- The commercial statement was relocated from Networking to README's Enterprise section (Omid Hekayati — decided).
- GP deliberately fixes no definitive identity-attestation or blocking method, so creativity is not foreclosed (Omid Hekayati — ruled).
- The critique's framing was corrected: IPv4/IPv6 has no accountability path at all — origins dissolve behind VPNs — whereas GP's router ranges have identifiable organizational owners, and the mandatory services exist precisely to exercise that accountability (Omid Hekayati).
- Interplanetary reach was confirmed to be an addressing/routability claim whose delivery belongs to lower layers (Omid Hekayati).
- The mandatory sRPC services were elevated to first-priority open work as the carrier of the whole accountability model (Omid Hekayati).
- QoS scheduling was likewise left unspecified as a ChaparKhane implementation concern (Omid Hekayati).
...
#### Deliberation
- The de-peering + signed reputation design was ruled to be recorded as a future option, not a fixed method, consistent with the deliberate openness of enforcement design (Omid Hekayati — ruled).
...
#### Deliberation
- The abbreviated bullet did not do justice to the discussed design; the full two-tier consensus, VPN bypass explanation, and graduated escalation must be recorded (Omid Hekayati — ruled).
...
#### Deliberation
- The critique was opened that GP was doing several jobs at once and paying real runtime cost for it (Omid Hekayati — requested).
- ChatGPT was partner model in the design review.
- The data-plane/control-plane and locator/identity separations were contributed (ChatGPT).
- The per-intermediary information-need framing — which intermediary needs which field — was contributed (ChatGPT).
- The "rare capability must not tax the common path" principle was contributed; it decided Planet's removal (ChatGPT).
- The Source-must-stay argument was contributed (ChatGPT).
- The ingress-vs-origin distinction behind source validation was contributed (ChatGPT).
- The last-deliverer responsibility model was contributed (ChatGPT).
- Through a full design review it was ruled that planetary location is reachability/topology knowledge about a society, not destination identity, so Planet left the address entirely (Omid Hekayati — decided).
- It was ruled that Thing-level and App-level delivery are two different destination concepts, realized as two independent self-contained frames, with single-application devices addressed at Thing level and no App bytes ever paid (Omid Hekayati — decided).
- It was ruled that field widths follow operational capacity of scope, not IPv6-inherited total address size — Thing ID reduced to 2 bytes (65,536 Things per router; ≈2⁴⁸ Things per society via hierarchy composition) (Omid Hekayati — decided).
- It was ruled that inter-society reachability is evaluated at Society + Router granularity, direct delivery is the encouraged norm, single mediation is legitimate with last-delivering-society responsibility, longer chains costly and discouraged, and physical hops never conflated with inter-society responsibility hops (Omid Hekayati — decided).
- It was ruled that source addresses are never dropped — they ground reply addressing and accountability (Omid Hekayati — decided).
- It was ruled that the standard-services roster is filled with reachability coordination, source validation, incident reporting to Router plus society authority, and router accountability — all as sRPC service calls, no GP control frame types (Omid Hekayati — decided).
- All document revisions were drafted from the review's conclusions and this document's edits (Super Z — applied).
...
#### Deliberation
- Network cost is transmission plus routing plus state, and GP's small headers must never be read as removing the obligation to keep data, processing, and storage local where locality reduces total cost (Omid Hekayati — decided).
- The principle was recorded at Networking level because it is an architecture-wide principle, not a GP packet property (Omid Hekayati — decided).
- The principle's placement and its non-mandate formulation — enable and incentivize locality; force no topology — were shaped (ChatGPT — collaborated).
...
#### Deliberation
- The earlier diagnosis of protocol versus architecture-concern separation is the framing the practice split and stack relocation apply (ChatGPT — collaborated).
- The Internet-transition procedure is a practice, not protocol — it evolves over time and belongs in the Practice facet (Omid Hekayati — decided).
- The stack overview belongs to the parent Networking document, since Giti's own copy read as if Giti were the whole stack (Omid Hekayati — decided).
- A fifth standard service must record the society-identifier allocation shape: governing-app announcement → temporary range → collective agreement for permanence under conditions such as minimum active routers or bandwidth (Omid Hekayati — decided).
- The mixed-level communication question resolves through os.md — the hypervisor and even the network driver are themselves independent applications that may hold App IDs, so a Thing-level endpoint is just an app that has not leased an App ID (Omid Hekayati — decided).
...
#### Deliberation
- ChatGPT was partner in the reviews whose conclusions these rulings apply.
- The practice document must cover every GP-related procedure, not the Internet bridge alone, and explanatory documents must not link to it — the transmission pointers were removed from giti.md (Omid Hekayati — decided).
- Routing knowledge must stay local: no router holds global reachability state — a router knows only its direct connections, hands packets to any router of the destination society when the destination router is not directly reachable, and relays for others only under prior agreement, which surfaces the routing-economy question (how relay cost is paid) as a new Unresolved question (Omid Hekayati — decided).
- "User" must not appear anywhere — the concept was deliberately renamed to app throughout Memar (as in os.md), so "first users" became "first apps" and service-number assignment was attributed to sRPC, not this document (Omid Hekayati — decided).
- A Thing-level endpoint is effective precisely when the device has a default app — the driver as the application endpoint — and whether an unexpected App-level packet is merely delivered or reported as an attack is that device app's own rule, not the protocol's (Omid Hekayati — decided).
...
#### Deliberation
- The practice document must cover every GP-related procedure rather than the Internet bridge alone — the bridge was an example, not the scope — and explanatory documents must never link to a practice document (Omid Hekayati — decided).
...
#### Deliberation
- Memar's own protocol specifications are collected in `docs/protocols/`, so a reader (or agent) needing only to *use* an implementation goes to the implementing repository instead of loading specification documents; this document is a protocol specification and moved with its siblings (Omid Hekayati - directed, decided).
