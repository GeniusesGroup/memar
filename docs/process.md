...
A **defect** is a failure observed against an expectation: a process instance whose enacted behavior contradicts the expectation its model stated for it (see *Expectations and Checks* and *Failure*, above). The concept presupposes the expectation — behavior can be contrary only to something stated — so where no expectation was recorded, the first act of resolution is stating the expectation the observed behavior is judged against, per the ordering that topic requires.
