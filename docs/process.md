...
A further public discussion — the design of Go's `context` package and its equivalents — recurred to the same pattern from the request side: a mechanism (a request-scoped context object) treated as though it were the natural carrier of cancellation, deadlines, and cross-layer data, its cost never examined before adoption. The analysis is recorded in *Requests, Cancellation, and Timeout* below.

Failure is treated here as a condition and outcome of a process, to the extent Process itself requires; the detailed conceptual treatment of Error remains out of scope and belongs to dedicated Error documentation.
...
A process model states what the process receives and what it must produce: the expected input, the expected output, the conditions under which each applies, and which outcomes count as which (see *Outcomes*, above). These expectations are part of the process model and, per *Process Definition and Process Instance*, precede every particular enactment — an instance can be checked against them precisely because they were stated first.

The expectations exist even when nobody writes them down. Before knowing how a process will be carried out, its participants already hold an expectation of what the process receives and what it must produce — if only tacitly, in temporary memory: one participant's attention, a conversation's context, an informal note. Tacit holding has two failures. The expectation is invisible to every participant other than its holder, and nothing can be checked against it, because no stated form of it exists. Writing the expected input and output down before enactment removes both failures at once, and forcing the expectation into words additionally surfaces the design's ambiguities while resolving them is still inexpensive — the same reasoning [Modeling → Modeling Before Implementation](./modeling.md#modeling-before-implementation) applies at the model level.

A **check** is the comparison of a process instance against the model's stated expectations. This document uses *check* for that comparison and *expectation* for its stated form; a domain's word often names both together — software's *test* commonly names the written expectation and its run alike, and documents that need the distinction use these two words rather than one overloaded domain term. The definition makes the concept's dependence on Process explicit: a check presupposes a process — a model whose expectations precede its instances — and without that structure testing has no subject. The definition also fixes the ordering. A statement of expectations produced after the enactment is not an expectation; it is a description read off the instance, able to confirm what was built yet unable to catch a departure from what was intended. For processes whose conformance matters — where participants must be able to determine whether an instance satisfied the intent — the expected input and output are therefore stated, in a form the domain can apply, before the enactment. [Protocol → Protocol, Science, and Methodology](./protocol.md#protocol-science-and-methodology) states the same requirement from the protocol side, as the condition for reproducibility and verification.

The realization of a check — the procedure, tool, or representation that performs the comparison — is a mechanism, selected per *Process Before Mechanism* after the expectations exist, and is not part of the concept. Three boundaries keep this discipline from overreaching. A process whose enactment is itself the means of discovery — an inquiry, an exploration — has its expectations stated for the inquiry process (what evidence it must produce, what decision it must return, what record it must leave), not for answers nobody yet has. Stating expectations for a process that was already enacted is **verification** — valuable, and not this discipline: verification still performs a check, but without the prior ordering that makes the check able to catch departure from stated intent, because that ordering cannot be recovered afterward. And a process that cannot be re-enacted — a one-shot enactment — still benefits from written expectations before it runs; that the check after enactment may be unavailable or only partially available does not void the ordering, and does not exempt the process from stating what it was meant to receive and produce.
...
A **defect** is a negative check verdict — a process instance whose enacted behavior contradicts its model's stated expectation (see *Expectations and Checks*, above). A defect is not the *Failure* outcome: an instance may complete and still be defective, or fail exactly as its model expected and not be. The concept presupposes the expectation — behavior can be contrary only to something stated — so where no expectation was recorded, the first act of resolution is stating the expectation the observed behavior is judged against, per the ordering that topic requires.

Resolving a defect whose cause is not yet known is not a development step. It is an **inquiry** — a process whose enactment is itself the means of discovery — and its expectations are therefore stated for the inquiry itself: what evidence it must produce (observations that discriminate among candidate causes), what decision it must return (the generating cause identified, or the inquiry closed without one), and what record it must leave. Stating expectations for answers nobody yet has is precisely the boundary case *Expectations and Checks* already excludes.

The two processes are conducted under different disciplines, and the ecosystem's single word for both — *debugging* — hides the seam. The inquiry runs on [Cognition](./cognition.md)'s operations: discrepancy noticing between observed and modeled behavior, receiving the reported defect at its conceptual root rather than only its reported instance ([Cognition → Discourse Norms](./cognition.md#discourse-norms-derived-from-this-model)), and the awareness disciplines — the characteristic failure of an investigation is confirmation, the discrepancy-noticing that only confirms the first candidate cause. Where the inquiry's course and outcome deserve to survive, it is recorded as a deliberate inquiry per [Documentation — Research](./documentation-research.md), negative and inconclusive outcomes included.

The correction the inquiry concludes with is an ordinary development step under *Expectations and Checks*: its expected input and output stated before the fix, the fixed instance checked against them. Where the finding is that the generating cause lives in the model rather than the implementation — behavior contradicting the model because the model is wrong — the correction follows [Modeling](./modeling.md)'s discipline, and the defect is evidence for that document's heuristic that behavioral complexity signals an immature model.
...
This document does not restate that treatment. What is specific to Process is only this: an execution Agent's responsibility for advancing part of a process is a relationship, not a redefinition of the process boundary, and a Worker, Actor, thread, or service instance is a representation of that responsibility, not the concept itself — see Concurrency, below, for where this becomes concrete.
...
A process that appears to operate on a shared resource may sometimes be redesigned so that the relevant work is partitioned by ownership, routed to a responsible participant, serialized by scheduling, or otherwise structured such that a lock is unnecessary — see *Agency and Execution Responsibility*, above, for how responsibility for a partition can be assigned to one execution Agent at a time without that Agent becoming a permanent domain owner.
...
What genuinely does vary, and does deserve modeling attention, is how many independent units of execution — Workers — a process's activities are distributed across, and how those Workers relate to physical execution resources. A Worker is a logical unit this model cares about; it may be registered and assigned its own identifier independently of how many CPU cores exist or which one it happens to run on at a given moment. A CPU core is a physical execution resource with its own identifier, assigned by the operating system or runtime rather than by the process model. A Worker taking responsibility for a bounded part of a process is a Concurrency-specific instance of a broader concept — see [Agency → Execution Agent](./agency.md#execution-agent) for the general treatment of a System assuming bounded execution responsibility, of which a Worker is one possible implementation representation, not the concept itself. The relationship between a Worker and a physical core — one Worker pinned to one core, several Workers time-sharing one core, one Worker migrating across cores — is exactly the kind of choice *Mechanisms Are Not Obligations* already warns should not be assumed by default: pinning part of a process to one fixed Worker, for instance, can resolve a concurrency hazard without a lock, precisely because it removes the possibility of two activities touching the same state through two different Workers at once. This is a modeling decision worth considering regardless of how many cores are involved, not a technique that only becomes relevant once multiple cores are.

#### Go's Scheduler as an Illustration
...
**Development** is a process in which an entity passes by degrees through successive stages toward a different stage — for Memar's purposes, typically a more advanced or mature one. The word's established general meaning already treats development as a kind of process; this document adopts it as such and adds no new structure: a development has an intent (the target stage its model states), participants (those who advance the entity and those affected by its advancement), activities and interactions, state (the current stage), and outcomes (the target reached, partial progress, redirection, abandonment) — all per the topics above. Progression is by degrees, not a single transformation, and not necessarily monotonic: a development may revisit earlier stages with new information, per *Continuation and Retry*, and its expected input and output are stated and checked per *Expectations and Checks*, above.

Development is accordingly carried as an umbrella term — a word that names no content of its own and serves only to gather concepts that already exist, like *object-oriented*, which adds nothing beyond encapsulation, inheritance, and polymorphism yet usefully names their combination, or *leader*, which bundles the distinct concepts management science separates. Its content here is therefore exactly the Process topics above: every question about a development is answered by applying them to the entity being advanced. The richer definitions scholars attach to the word — one tradition, for instance, defines development as the growth of a system's problem-solving power — are candidate characterizations of the target stage and the measure of progress toward it, decisions a particular development's model must make; they do not alter the concept, and adjudicating among them belongs to the model, not to this topic.

The concept is domain-independent. Memar's concern is chiefly the development of systems, software among them: the developed entity is a system, its stages are recorded in its models ([Modeling](./modeling.md)), and the development is carried out by a network of agents — see [Agency → Agency and Development](./agency.md#agency-and-development) for that treatment, which is not restated here. Because Memar is a development framework in [Framework](./framework.md)'s sense — its described system is intended to guide the development of other systems — Development is the process Memar's own documentation participates in producing. The rules for how development processes are conducted, including how their expectations are written and checked, are Memar's protocols: owned by the protocol layer, named here without citation, per that layer's citation rule. This document supplies the process concepts such protocols govern.
...
- **Agency** describes the capacity and structure through which systems can act and assume responsibility; Process uses this concept to reason about which acting systems may be responsible for advancing particular parts of a progression. See [Agency](./agency.md) for the full treatment.
