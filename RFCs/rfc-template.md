...
Contributor(s):
  - Name: "Omid Hekayati"
    ...
    Tasks:
      - Titles: ["RFC Template Specification"]
        URI: ""
        Explanation: "Directed every revision; resolved the merge-numbering, collision-resolution, and Contribution-to-Task decisions; caught multiple front-matter inconsistencies."
  - Name: "Claude"
    URI: "https://claude.ai"
    ...
    Tasks:
      - Titles: ["RFC Template Specification"]
        URI: ""
        Explanation: "Drafted and revised this specification across multiple rounds; proposed the topic-first Reference-level structure and Discussion section after finding scattered content in a real RFC; migrated Contributor(s) to Task-based tracking."
  - Name: "Super Z"
    URI: "https://z.ai"
    Model: "GLM 5.2"
    Effort: "Medium"
    Tasks:
      - Titles: ["RFC Template Specification"]
        URI: ""
        Explanation: "Restructured an earlier revision to follow the template's own section layout and applied the pre-Final merging and contributor-preservation rules."
---
...
3. Fill in the rest of the front matter:
   ...
   - `Contributor(s)`: one entry per contributor, each with at least one `Tasks` item (`Titles`, optional `URI`, a short, headline-style `Explanation`). AI contributors also add `Model` and `Effort`.
4. Write the body. `Summary`, `Motivation`, and `Guide-level explanation` stay unified, RFC-wide prose. Under `Reference-level explanation`, split the content into topic subsections (`### {Section title}`); give each topic its own `#### Drawbacks` / `Rationale and alternatives` / `Prior art` / `Unresolved questions` / `Future possibilities` / `Suggested Naming Conventions` only where that topic genuinely has something to say for it — never as an empty header. If a topic needs finer sub-sections, nest `#### {Sub section title}` under it, with a sibling `#### Discussion` collecting anything about the topic as a whole rather than one sub-section — the same pattern one level down. Anything that applies to the whole RFC rather than one topic goes in the RFC-wide `## Discussion` section, not into any topic's own bundle.
...
**Required.** Every contributor entry must have at least one `Tasks` entry. A `Tasks` entry's `URI` is optional — include it when a supporting artifact genuinely exists and is reachable; when it doesn't, `Explanation` alone (e.g. "present for the design discussion, no separate artifact") is sufficient and still required.
  ...
  - Name: "My Name"
    ...
    Tasks:
      - Titles: ["Design review"]
        URI: ""
        Explanation: "Present for the design discussion; did not directly edit the text"
  ...
  - Name: "Claude"
    ...
    Tasks:
      - Titles: ["Initial draft"]
        ...
        Explanation: "Drafted initial text; incorporated revisions"
```
...
- **A dedicated `Contribution` field (an earlier draft; not chosen)**: once `Tasks` gained real structure (`Titles`/`URI`/`Explanation`), a separate free-text field became redundant — anything expressible in `Contribution` is expressible as a `Tasks` entry's `Explanation`, including simple presence with no artifact, since `URI` there is optional.
...
##### Prior art
This field's use of `URI` for identity follows RFC 000014's use of File URIs for dependency resolution. The CRediT (Contributor Roles Taxonomy) system used by academic journals is a comparable example of structured contributor attribution outside this project — though CRediT uses a fixed, controlled set of role categories, which this specification deliberately does not adopt for `Explanation`, since a contributor's actual involvement can be broader or more specific than any fixed list allows.

##### Unresolved questions
1. Should `Explanation` eventually move to a fixed, controlled set of categories, or stay free text indefinitely?
...
- **Reference-level explanation**: the normative section, organized as topic subsections (`### {Section title}`), each optionally carrying its own `#### Drawbacks`, `Rationale and alternatives`, `Prior art`, `Unresolved questions`, `Future possibilities`, `Suggested Naming Conventions` — included only where that topic actually has content for them. A topic needing finer sub-sections nests `#### {Sub section title}` under it, with a sibling `#### Discussion` (and `#####` bundles) for anything about the topic as a whole — the same topic/Discussion split repeated one level down.
...
- **Change Rationale**: why the RFC changed across revisions — kept separate from Discussion, since it is about the document's own history, not a justification of the current design, and accumulates append-only much like `Contributor(s)`.
...
2. Is `Change Rationale` needed at all, given that version control already records exactly what changed and when? Kept for now at the top level rather than removed, pending a decision. This is reinforced by a related question raised while refining the Contributor(s) preservation rule: whether material pruned from a stale contributor entry should be preserved in `Change Rationale` — if version control already covers that case adequately, it is one more reason `Change Rationale` may be redundant.

### Future possibilities
If tooling is ever built to help produce or check RFCs, it could read this single spec file and walk a contributor through each front-matter field and body section in order, validating values as they're filled in. If a future RFC introduces an additional `Status` value, `Related RFCs` `Reason` value, or `Contributor(s)` field, this specification is where that addition should be documented.
