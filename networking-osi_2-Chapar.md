...
## Discovery
A new device announces itself by sending a Broadcast frame (`HopCount == 0x00`). As the frame is flooded hop by hop, each intermediate switch rewrites its receiving port into the frame's header per the existing port-rewrite [Rule](#rules) below — so the frame accumulates its own reverse path as it travels.

Any node **may** respond (optional); ChaparKhane **must** respond. A response is sent Unicast, using the accumulated hop-port sequence — reversed — directly as the path back to the new device; no separate route-computation step is needed.

Re-establishing a path after a topology change (see the "Wiring changes" Non-Goal in [RFC 001000](./RFCs/001000-chapar-introduction-goals-and-topology.md)) is not handled by a dedicated Chapar mechanism: since no Chapar frame travels without an accompanying higher-layer packet, an ordinary upper-layer timeout on a broken path is expected to trigger a fresh Discovery. ChaparKhane may optionally notify authorized upper-layer subscribers of a known path change, as an additional (not required) optimization.

Whether a device responds to Discovery at all is a policy decision that belongs to a higher layer or the host OS (comparable to a "network discovery" toggle in general-purpose operating systems), not something Chapar mandates. Once a device has a path to ChaparKhane, ChaparKhane can also compose a path between it and any other already-discovered device — see [RFC 001003](./RFCs/001003-chapar-discovery-and-path-establishment.md) for how, and for the full rationale and alternatives considered.
